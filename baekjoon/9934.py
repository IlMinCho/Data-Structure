...
def find_levels(k, visit_order):
    levels = [[] for _ in range(k)]
    
    def assign_levels(start, end, depth):
        if start > end or depth >= k:
            return
        mid = (start + end) // 2
        levels[depth].append(visit_order[mid])
        assign_levels(start, mid - 1, depth + 1)
        assign_levels(mid + 1, end, depth + 1)
    
    assign_levels(0, 2 ** k - 2, 0)
    return levels
...
def main():
    ...
    levels = find_levels(k, visit_order)
    for level in levels:
