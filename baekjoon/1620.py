...
def create_pokemon_dict(n):
    pokemon_dict = {}
    reverse_pokemon_dict = {}
    for i in range(1, n + 1):
        pokemon_name = sys.stdin.readline().strip()
        pokemon_dict[i] = pokemon_name
        reverse_pokemon_dict[pokemon_name] = i
    return pokemon_dict, reverse_pokemon_dict

def answer_questions(pokemon_dict, reverse_pokemon_dict, m):
    answers = []
    for _ in range(m):
        question = sys.stdin.readline().strip()
        if question.isdigit():
            answers.append(pokemon_dict[int(question)])
        else:
            answers.append(str(reverse_pokemon_dict[question]))
    return answers

def main():
    n, m = map(int, sys.stdin.readline().strip().split())
    pokemon_dict, reverse_pokemon_dict = create_pokemon_dict(n)
    answers = answer_questions(pokemon_dict, reverse_pokemon_dict, m)
    sys.stdout.write('\n'.join(answers))
